...
function AppContent() {
 ...
 const [page, setPage] = useState<Page>(() => previewFirstRun ? 'settings' : 'chat'); const [settingsTab, setSettingsTab] = useState<SettingsTab>(() => previewFirstRun ? 'agents' : 'general');
  ...
  return <main className={`cockpit ${page === 'tasks' || page === 'archive' ? 'tasks-mode' : page === 'forge' ? 'forge-mode' : page === 'settings' ? 'settings-mode' : activeMod ? 'mod-mode' : ''}`} data-theme={theme} style={style}>
    <header className="app-header">
      ...
      <nav className="page-tabs" aria-label="Primary navigation">
        {(['chat', 'tasks', 'archive', 'forge'] as Page[]).map((item) => <button className={!activeMod && page === item ? 'active' : ''} onClick={() => changePage(item)} key={item} aria-current={!activeMod && page === item ? 'page' : undefined}>{item === 'forge' ? 'Forge' : item}</button>)}
