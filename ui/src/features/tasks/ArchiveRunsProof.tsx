...
type ChildVerification = NonNullable<ArchiveRunDetail['subagents'][number]['verification']>;
const childVerificationStatus: Record<ChildVerification['status'], { label: string; className: string }> = {
  passed: { label: 'verification passed', className: 'proof-status-completed' },
  failed: { label: 'verification failed', className: 'proof-status-failed' },
  failed_expected: { label: 'verification failed as expected', className: 'proof-status-warning' },
  not_run: { label: 'verification not run', className: '' },
};
...
function ProofDetail({ run }: { run: ArchiveRunDetail }) {
  ...
  return <div className="archive-proof-detail">
    ...
    {run.request ? <section className="proof-final"><h3>Your request</h3><ReactMarkdown remarkPlugins={markdownPlugins} components={{ table: MarkdownTable }}>{run.request}</ReactMarkdown></section> : null}
