...
export function Tasks({ agents }: { agents: Agent[] }) {
  ...
  async function saveProject(event: FormEvent<HTMLFormElement>) {
    ...
    const body = { name: projectDraft.name, description: projectDraft.description };
    setSaving(true);
    setError('');
    try {
      if (projectDraft.id) {
  ...
  return <div className="tasks-page-shell"><div className="page-view tasks-page">
    ...
    {!loading && !projects.length && <section className="task-empty-state"><h2>No projects yet</h2><p>Create a project before adding work to the board.</p><button className="primary" type="button" onClick={() => { setShowProjects(true); setProjectDraft({ name: '', description: '' }); }}>Add project</button></section>}
