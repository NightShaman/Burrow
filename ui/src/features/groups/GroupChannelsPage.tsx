  ...
  return <div className="group-page">
    <main className="group-transcript">
      ...
      <div className="group-composer">
        ...
        <ChatComposer draft={message} setDraft={composerHistory.setDraft} onKeyDown={handleComposerKeyDown} attached={attached} onAttach={attachImage} onRemoveAttachment={(index) => setAttached((current) => current.filter((_, attachmentIndex) => attachmentIndex !== index))} disabled={sending} onSend={() => void send()} conversationTurns={channel.turns.map((turn): SessionTurn => ({ role: turn.role, content: turn.content, ts: turn.createdAt, metadata: { ...turn.metadata, ...(turn.authorId ? { fromAgentId: turn.authorId } : {}), ...(turn.authorName ? { fromAgentName: turn.authorName } : {}) } }))} operatorName={operatorIdentity?.name || operator?.name || 'You'} agentNames={new Map(Array.from(agentById, ([id, agent]) => [id, agent.name]))} placeholder="Message the group…" />
      </div>
    </main>
  </div>;
}
