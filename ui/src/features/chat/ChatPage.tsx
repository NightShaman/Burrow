...
export function ChatModelSelector({ selected, savedProviders, updateAgent, sessions, sessionId, onSessionChange, onNewSession, onNewNamedSession, onCreateGroup, locked = false }: { selected: Agent; savedProviders: SavedProvider[]; updateAgent: (patch: Partial<Agent>) => void; sessions: { id: string }[]; sessionId: string; onSessionChange: (sessionId: string) => void; onNewSession: () => void; onNewNamedSession: () => void; onCreateGroup: () => void; locked?: boolean }) {
  ...
  return <div className="workspace-toolbar"><label>Provider <select value={provider?.provider ?? ''} onChange={(e) => chooseProvider(e.target.value)} disabled={locked || !savedProviders.length}>{!savedProviders.length && <option value="">Configured runtime</option>}{savedProviders.map((item) => <option value={item.provider} key={item.id}>{item.provider}</option>)}</select></label><label>Model <select value={model} onChange={(e) => chooseModel(e.target.value)} disabled={locked || !models.length}>{!models.length && <option value="">Configured runtime</option>}{models.map((item) => <option key={item} value={item}>{provider?.modelLabels?.[item] ?? item}</option>)}</select></label><label>Effort <select value={effort} onChange={(e) => updateAgent({ effort: e.target.value })} disabled={locked || !efforts.length}>{!efforts.length && <option value="">Not configured</option>}{efforts.map((item) => <option key={item}>{item}</option>)}</select></label><label className="temperature-control">Temp <span>{temperature.toFixed(1)}</span><input type="range" min="0" max="2" step="0.1" value={temperature} onChange={(e) => chooseTemperature(e.target.value)} disabled={locked || !provider} /></label><div className="session-controls"><button className="new-session group-chat-trigger" onClick={onCreateGroup} disabled={locked}>Group Chat</button><button className="new-session" onClick={onNewNamedSession} disabled={locked}>New session</button><label aria-label="Session"><select value={sessionId} onChange={(e) => onSessionChange(e.target.value)} disabled={locked || !sessions.length}>{sessions.map((session) => <option key={session.id} value={session.id}>{session.id}</option>)}</select></label><button className="new-session" onClick={onNewSession} disabled={locked}>Reset Session</button></div></div>;
}

export function Chat({ selected, parent, operator, draft, setDraft, attached, onAttach, onRemoveAttachment, isNewSession, turns, isLoading, error, isSending, activeRunId, activeToolActivity, liveProgress, liveAnswer, onSend, onCancel }: { selected: Agent | Subagent; parent: Agent; operator: OperatorProfile; draft: string; setDraft: (value: string) => void; attached: ChatAttachment[]; onAttach: (files: File[]) => void; onRemoveAttachment: (index: number) => void; isNewSession: boolean; turns: SessionTurn[]; isLoading: boolean; error: string; isSending: boolean; activeRunId: string; activeToolActivity?: ToolActivity; liveProgress: ProgressEntry[]; liveAnswer: string; onSend: () => void; onCancel: () => void }) {
  const composerHistory = useComposerHistory(draft, turns.filter((turn) => turn.role === 'user').map((turn) => turn.content ?? ''), setDraft);
  return <div className="chat-view">
    <ChatTranscript selected={selected} parent={parent} operator={operator} isNewSession={isNewSession} turns={turns} isLoading={isLoading} error={error} isSending={isSending} activeRunId={activeRunId} activeToolActivity={activeToolActivity} liveProgress={liveProgress} liveAnswer={liveAnswer} />
