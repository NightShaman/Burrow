...
describe('normalizeModManagement', () => {
  ...
  it('normalizes missing or invalid payloads to empty collections', () => {
    expect(normalizeModManagement(null)).toEqual({ ok: undefined, restartRequired: false, mods: [], sources: [] });
    expect(normalizeModManagement({ mods: {}, sources: 'bad' })).toEqual({ ok: undefined, restartRequired: false, mods: [], sources: [] });
  });
