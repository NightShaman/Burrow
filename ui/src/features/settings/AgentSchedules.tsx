...
export function AgentSchedules({ agentId, targets, savedProviders, overflowTarget }: { agentId: string; targets: ApiTarget[]; savedProviders: SavedProvider[]; overflowTarget?: HTMLElement | null }) {
  ...
  const reset = () => { setEditingId(null); setName(''); setPrompt(''); setCron('0 9 * * *'); setTimezone(Intl.DateTimeFormat().resolvedOptions().timeZone || 'UTC'); setEnabled(true); setJobModel({ modelConnectionId: null, model: null }); setError(''); };
  const edit = (job: ScheduledJob) => { setEditingId(job.id); setName(job.name); setPrompt(job.prompt); setCron(job.cron); setTimezone(job.timezone); setEnabled(job.enabled); setJobModel({ modelConnectionId: job.modelConnectionId ?? null, model: job.model ?? null }); setError(''); };
  const save = async () => {
