...
type Props = { agents: Agent[]; targets?: unknown[]; onAgentsChanged: () => Promise<void>; section?: RemoteNodesSection; overflowTarget?: HTMLElement | null };
...
export function RemoteNodesSettings({ agents, onAgentsChanged, section = 'controller', overflowTarget: externalOverflowTarget }: Props) {
  ...
  const overflowTarget = externalOverflowTarget;
  ...
  const selectedGateway = trust.find(item => item.gatewayId === selectedGatewayId) ?? trust[0];
  const selectedGatewayConnection = selectedGateway ? gateways.find(g => g.gatewayId === selectedGateway.gatewayId) : undefined;
  const selectedAgent = agents.find(agent => agent.id === selectedAgentId) ?? agents[0];
  const approvedGateways = trust.filter(item => { const connection = gateways.find(g => g.gatewayId === item.gatewayId); return trustState(item, connection).approved && !trustState(item, connection).revoked; });
  useEffect(() => { if (selectedGateway && selectedGateway.gatewayId !== selectedGatewayId) setSelectedGatewayId(selectedGateway.gatewayId); }, [selectedGateway?.gatewayId, selectedGatewayId]);
  useEffect(() => { if (selectedAgent && selectedAgent.id !== selectedAgentId) setSelectedAgentId(selectedAgent.id); }, [selectedAgent?.id, selectedAgentId]);
  return <div className="remote-nodes-settings">
    ...
    {section === 'gateways' && <>
      <SettingSection title="Configured gateways"><p className="settings-description">Choose a gateway to inspect its trust, health, and actions. Disconnected gateways remain visible.</p>{trust.length === 0 ? <p className="settings-empty">No gateways enrolled yet.</p> : <div className="remote-gateway-selector">{trust.map(item => { const connection = gateways.find(g => g.gatewayId === item.gatewayId); const state = trustState(item, connection); return <button type="button" className={selectedGateway?.gatewayId === item.gatewayId ? 'active' : ''} aria-pressed={selectedGateway?.gatewayId === item.gatewayId} onClick={() => setSelectedGatewayId(item.gatewayId)} key={item.gatewayId}><strong>{item.gatewayId}</strong><small>{state.label} · {state.connected ? 'Connected' : 'Disconnected'}</small></button>; })}</div>}{error && <p className="settings-request-error" role="alert">{error}</p>}<div className="remote-gateway-enroll"><strong>Enroll or rotate gateway</strong><div className="field-pair"><Field label="Gateway ID"><input value={gatewayId} onChange={e => setGatewayId(e.target.value)} placeholder="host-123" /></Field><Field label="Controller ID"><input value={controllerId} onChange={e => setControllerId(e.target.value)} /></Field></div><Field label="Enrollment / rotation secret"><input type="password" value={secret} onChange={e => setSecret(e.target.value)} placeholder="One-time secret input" /></Field><div className="setting-actions"><button className="primary" type="button" disabled={Boolean(busy)} onClick={saveGateway}>{busy === 'gateway' ? 'Saving…' : 'Enroll or rotate gateway'}</button></div></div></SettingSection>
      {overflowTarget && createPortal(<div className="settings-overflow-content remote-gateway-overflow">{selectedGateway ? (() => { const state = trustState(selectedGateway, selectedGatewayConnection); return <SettingSection title={selectedGateway.gatewayId}><div className={`remote-gateway-detail ${state.revoked ? 'revoked' : state.connected ? 'connected' : ''}`}><strong>{state.label}</strong><small>{selectedGateway.controllerId} · {state.connected ? `Connected · ${selectedGatewayConnection?.status || 'authenticated'}` : 'Disconnected'}</small>{selectedGatewayConnection && <><small>{selectedGatewayConnection.name || 'Gateway daemon'}{selectedGatewayConnection.version ? ` · v${selectedGatewayConnection.version}` : ''}{selectedGatewayConnection.protocolVersion ? ` · protocol ${selectedGatewayConnection.protocolVersion}` : ''}</small><small>Connected {formatTime(selectedGatewayConnection.connectedAt)} · Last seen {formatTime(selectedGatewayConnection.lastSeenAt)}</small><small>{selectedGatewayConnection.activeOperations?.length ?? 0} active operation{selectedGatewayConnection.activeOperations?.length === 1 ? '' : 's'}</small></>}</div><button className="danger" type="button" onClick={() => revoke(selectedGateway.gatewayId)} disabled={Boolean(busy) || state.revoked}>Revoke gateway</button></SettingSection>; })() : <><strong>Gateway details</strong><p className="settings-empty">Select a configured gateway.</p></>}</div>, overflowTarget)}
    </>}
    {section === 'assignments' && <>
      <SettingSection title="Agent execution environments"><p className="settings-description">Choose an agent to configure its controller-owned environment for future turns.</p>{agents.length === 0 ? <p className="settings-empty">No agents configured.</p> : <div className="remote-agent-selector">{agents.map(agent => <button type="button" className={selectedAgent?.id === agent.id ? 'active' : ''} aria-pressed={selectedAgent?.id === agent.id} onClick={() => setSelectedAgentId(agent.id)} key={agent.id}><strong>{agent.name}</strong><small>{agent.executionEnvironment?.kind === 'gateway' ? agent.executionEnvironment.hostId : 'Local controller'}</small></button>)}</div>}{error && <p className="settings-request-error" role="alert">{error}</p>}</SettingSection>
      {overflowTarget && createPortal(<div className="settings-overflow-content remote-agent-overflow">{selectedAgent ? <SettingSection title={`${selectedAgent.name} environment`}><p className="settings-description">The model cannot switch hosts mid-turn.</p><AgentEnvironment agent={selectedAgent} gateways={approvedGateways} onSaved={onAgentsChanged} onError={setError} /></SettingSection> : <><strong>Agent assignment</strong><p className="settings-empty">Select an agent to configure its environment.</p></>}</div>, overflowTarget)}
    </>}
