...
export function RailPanelSettings({ side, layout, setLayout, singlePanel, setSinglePanel, topPanel, setTopPanel, bottomPanel, setBottomPanel, onPanelSelected }: {
  ...
  onPanelSelected?: (panel: PanelId) => void;
}) {
  const panelSelect = (label: string, value: PanelId, onChange: (panel: PanelId) => void) => <Field label={`${side} · ${label}`}><select value={value} onFocus={() => onPanelSelected?.(value)} onChange={(event) => { const panel = event.target.value as PanelId; onChange(panel); onPanelSelected?.(panel); }}>{panelRegistry.map((panel) => <option key={panel.id} value={panel.id}>{panel.label}</option>)}</select></Field>;
  return <div>
