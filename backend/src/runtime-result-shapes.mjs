...
export function summarizeToolResults(toolResults = []) {
  return (toolResults || []).map((toolResult, index) => ({
    ...
    ...(toolResult.tool?.startsWith('forge_') ? {
      ...
      models: Array.isArray(toolResult.models) ? toolResult.models.slice(0, RECEIPT_TEXT_LIMITS.list).map((m) => ({ connectionId: m?.connectionId || null, modelId: m?.modelId || null, label: compactText(m?.label, 240), kind: m?.kind || null, available: m?.available === true, unavailableReason: m?.unavailableReason || null })) : undefined,
      music: toolResult.music && typeof toolResult.music === 'object' ? { available: toolResult.music.available === true, reason: toolResult.music.reason || null, models: Array.isArray(toolResult.music.models) ? toolResult.music.models.slice(0, RECEIPT_TEXT_LIMITS.list).map((m) => ({ connectionId: m?.connectionId || null, modelId: m?.modelId || null, label: compactText(m?.label, 240), kind: m?.kind || null, available: m?.available === true, unavailableReason: m?.unavailableReason || null })) : undefined } : undefined,
      video: toolResult.video || undefined,
      sourceAttachments: toolResult.sourceAttachments || undefined,
      jobs: Array.isArray(toolResult.jobs) ? toolResult.jobs.slice(0, RECEIPT_TEXT_LIMITS.list).map((j) => ({ id: j?.id || null, mode: j?.mode || null, status: j?.status || null, error: j?.error || null })) : undefined,
      job: toolResult.job && typeof toolResult.job === 'object' ? { id: toolResult.job.id || null, mode: toolResult.job.mode || null, status: toolResult.job.status || null, error: toolResult.job.error || null, artifacts: Array.isArray(toolResult.job.artifacts) ? toolResult.job.artifacts.slice(0, RECEIPT_TEXT_LIMITS.evidence).map((a) => ({ id: a?.id || null, kind: a?.kind || null, name: a?.name || null, mimeType: a?.mimeType || null, previewUrl: a?.previewUrl || null, downloadUrl: a?.downloadUrl || null })) : [] } : undefined,
    } : {}),
    ...
    execution: toolResult.execution && typeof toolResult.execution === 'object' ? {
      kind: toolResult.execution.kind === 'remote' ? 'remote' : 'local',
      providerId: toolResult.execution.kind === 'remote' ? compactText(toolResult.execution.providerId, 120) || null : undefined,
      targetId: toolResult.execution.kind === 'remote' ? compactText(toolResult.execution.targetId, 240) || null : undefined,
      parentRunId: compactText(toolResult.execution.parentRunId, 240) || null,
      toolCallId: compactText(toolResult.execution.toolCallId, 240) || null,
      operationId: compactText(toolResult.execution.operationId, 240) || null,
    } : undefined,
