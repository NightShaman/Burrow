...
export class McpSettingsStore {
  ...
  prepareSave(value = {}) { if (text(value.id).startsWith('mod.') || text(value.baseUrl).startsWith('mod://')) throw new Error('mcp_mod_connection_managed'); const connection = input(value); const connectionId = value.id ? id(value.id, 'mcp_connection_id') : randomUUID(); return { connectionId, existing: this.get(connectionId), next: { id: connectionId, ...connection } }; }
  ...
  refreshTools(connectionId, discoveredTools = []) { if (text(connectionId).startsWith('mod.')) throw new Error('mcp_mod_connection_managed'); const connection = this.get(connectionId); if (!connection) throw new Error('mcp_connection_not_found'); return this.save({ ...connection, tools: discoveredTools }); }
  remove(connectionId) { if (text(connectionId).startsWith('mod.')) throw new Error('mcp_mod_connection_managed'); return this.db.prepare('DELETE FROM mcp_connections WHERE id=?').run(id(connectionId, 'mcp_connection_id')).changes > 0; }
