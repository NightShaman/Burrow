...
const SAFE_FORGE_ERRORS = new Set([
  ...
  'source_attachments_unsupported', 'generation_options_unsupported', 'generation_failed',
]);
...
export const forgeToolSchemas = [
 { type:'function', function:{ name:'forge_catalog', description:'Discover available Forge image and speech models. Video, music, source attachments, and provider-specific tuning are honestly reported unsupported.', parameters:{type:'object',additionalProperties:false,properties:{reason:{type:'string'}}} } },
 { type:'function', function:{ name:'forge_create_job', description:'Explicitly request a potentially paid asynchronous Forge generation. Never use this without the operator asking for generation. Poll forge_inspect_job until the job reaches a terminal status; then attach a succeeded artifact.', parameters:{type:'object',additionalProperties:false,properties:{connectionId:{type:'string'},modelId:{type:'string'},prompt:{type:'string'},idempotencyKey:{type:'string'},reason:{type:'string'}},required:['connectionId','modelId','prompt','idempotencyKey']} } },
...
export async function executeForgeTool(name, args, context = {}) {
 try {
  ...
  if(name==='forge_create_job') return {ok:true,...await store.create({connectionId:args.connectionId,modelId:args.modelId,prompt:args.prompt,idempotencyKey:args.idempotencyKey})};
